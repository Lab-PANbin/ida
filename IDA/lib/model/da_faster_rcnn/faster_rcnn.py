...
#IDA loss
class ida_loss(nn.Module):
    def __init__(self):
        super().__init__()
        
    def forward(self, p, q):
        p_q_abs = torch.abs(torch.sub(p, q))
        p_q_max = torch.max(p, q)
        p_q_min = torch.min(p, q)
        ida_final_loss = torch.mean(torch.div(p_q_abs, (p_q_abs + 0.7 * p_q_max + 0.3 * p_q_min)))
        return ida_final_loss
...
class _fasterRCNN(nn.Module):
    ...
    def __init__(self, classes, class_agnostic, in_channel=4096):
        ...
        self.consistency_loss = ida_loss()
    ...
    def forward(
        ...
        if not (need_backprop.detach() == 0 and tgt_need_backprop.detach() == 1):
            need_backprop = torch.Tensor([0]).cuda()
            tgt_need_backprop = torch.Tensor([1]).cuda()

        assert need_backprop.detach() == 0 and tgt_need_backprop.detach() == 1
